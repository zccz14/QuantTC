...
namespace QuantTC.Indicators.Collectors
{
    ...
    {
        /// <inheritdoc />
        public WinningProbability(IIndicator<double> netValue, IIndicator<int> winningCounter,
            IIndicator<int> operationCounter)
        {
            NetValue = netValue;
    ...
    public class OperationCounter : Indicator<int>
    {
        ...
        private void NetValueOnUpdate()
        {
            ...
            Data.FillRange(Count, NetValue.Count,
                i => i == 0 ? 0 : (NetValue[i] != NetValue[i - 1] ? Data[i - 1] + 1 : Data[i - 1]));
            FollowUp();
        }
